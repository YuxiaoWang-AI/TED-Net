...
class HICODetection(torch.utils.data.Dataset):
    def __init__(self, img_set, img_folder, anno_file, clip_feats_folder, transforms, num_queries, args):
        ...
        if img_set == 'train':
            with open("./HICO-Det-IC-train.txt", "r") as f:
                filename_list = eval(f.read())
            print(len(filename_list))
            print(len(list(set(filename_list))))

            for index in range(len(self.annotations) - 1, -1, -1):
                a = self.annotations[index]
                if self.annotations[index]['file_name'] not in filename_list:
            ...
            with open("./HICO-Det-IC-test.txt", "r") as f:
                filename_list = eval(f.read())
            print(len(filename_list))
            for index in range(len(self.annotations) - 1, -1, -1):
                # print(self.annotations[index]['file_name'])
                a = self.annotations[index]
                if self.annotations[index]['file_name'] not in filename_list:
                    self.annotations.pop(index)
            print(len(self.annotations))
