...
class GEN_VLKT(nn.Module):
    def __init__(self, backbone, transformer, num_queries, aux_loss=False, args=None):
        ...
        self.queries = nn.Linear(hidden_dim, num_queries)
        self.queries.requires_grad = False

        self.h_w = nn.Linear(hidden_dim, hidden_dim)
        self.o_w = nn.Linear(hidden_dim, hidden_dim)
        self.i_w = nn.Linear(hidden_dim, hidden_dim)
        ...
        self.bn_fc = nn.LayerNorm(12)
        self.bn_obj = nn.LayerNorm(num_obj_classes + 1)
        self.hoi_class_fc = nn.Sequential(
            nn.Linear(hidden_dim + 12 + 2+ num_obj_classes + 1, args.clip_embed_dim),
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.h_w.weight)
        nn.init.xavier_uniform_(self.o_w.weight)
    ...
    def forward(self, samples: NestedTensor, is_training=True):
        if not isinstance(samples, NestedTensor): # False
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = self.backbone(samples) # resnet

        src, mask = features[-1].decompose()
        assert mask is not None
        # print(self.queries.weight.shape)
        # exit()
        query_embed_h, query_embed_o, query_embed_i = nn.functional.relu(torch.mm(self.queries.weight, self.h_w.weight)), nn.functional.relu(torch.mm(self.queries.weight, self.o_w.weight)), nn.functional.relu(torch.mm(self.queries.weight, self.i_w.weight))
        h_hs, o_hs, inter_hs = self.transformer(self.input_proj(src), mask,
                                                ...
                                                pos[-1])[:3]
        # h_hs torch.Size([3, 2, 64, 256]), o_hs torch.Size([3, 2, 64, 256]),
        outputs_sub_coord = self.hum_bbox_embed(h_hs).sigmoid() # torch.Size([3, 2, 64, 4])
        outputs_obj_coord = self.obj_bbox_embed(o_hs).sigmoid() # torch.Size([3, 2, 64, 4])
          
        # print(output_sub_obj_totol.requires_grad)
        if self.args.with_obj_clip_label: # True
            obj_logit_scale = self.obj_logit_scale.exp() # 14.2857
            o_hs = self.obj_class_fc(o_hs) # (3, 2, 64, 512)
            o_hs = o_hs / o_hs.norm(dim=-1, keepdim=True)
            outputs_obj_class = obj_logit_scale * self.obj_visual_projection(o_hs) # (3, 2, 64, 81)
        else:
            outputs_obj_class = self.obj_class_embed(o_hs)

        # with torch.no_grad():
        outputs_sub_box_center = outputs_sub_coord[:, :, :, :2]
        outputs_obj_box_center = outputs_obj_coord[:, :, :, :2]
        outputs_sub_box_w_h = outputs_sub_coord[:, :, :, 2:]
        outputs_obj_box_w_h = outputs_obj_coord[:, :, :, 2:]
        direction_obj2sub = outputs_sub_box_center - outputs_obj_box_center
        relative_pos = direction_obj2sub.clone()
        relative_pos[relative_pos >= 0] = 1.0
        relative_pos[relative_pos < 0] = -1.0
        
        
        n_l, b, n_queries, _ = outputs_obj_coord.shape
        sub_obj_box_iou = box_iou(box_cxcywh_to_xyxy(outputs_sub_coord).view(-1, 4), box_cxcywh_to_xyxy(outputs_obj_coord).view(-1, 4))
        sub_obj_box_iou = torch.diag(sub_obj_box_iou).view(n_l, b, n_queries).unsqueeze(-1)

        sub_div_obj_area = ((outputs_sub_box_w_h[:, :, :, 0] * outputs_sub_box_w_h[:, :, :, 1]) / (outputs_obj_box_w_h[:, :, :, 0] * outputs_obj_box_w_h[:, :, :, 1] + 1e-7)).unsqueeze(-1)
        output_sub_obj_totol = self.bn_fc(torch.cat([direction_obj2sub, sub_obj_box_iou, sub_div_obj_area, outputs_sub_coord, outputs_obj_coord], dim=-1))

        argmax = torch.argmax(outputs_obj_class, dim=-1)
        
        one_hot = F.one_hot(argmax, num_classes=self.num_obj_classes + 1).float()
        
        output_sub_obj_totol = torch.cat([output_sub_obj_totol, relative_pos, one_hot], dim=-1)
        
        
        if self.args.with_clip_label: # True
            logit_scale = self.logit_scale.exp()
            inter_hs = self.hoi_class_fc(torch.cat([output_sub_obj_totol, inter_hs], dim=-1))
    ...
    def _set_aux_loss_triplet(self, outputs_hoi_class, outputs_obj_class,
        ...
        if outputs_inter_hs is not None: # True
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_sub_obj_boxes(self, outputs, targets, indices, num_interactions):
        ...
        idx = self._get_src_permutation_idx(indices)
        src_sub_boxes = outputs['pred_sub_boxes'][idx] # (10, 4)
        src_obj_boxes = outputs['pred_obj_boxes'][idx] # (10, 4)
        target_sub_boxes = torch.cat([t['sub_boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0) # (10, 4)
        target_obj_boxes = torch.cat([t['obj_boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0) # (10, 4)
    ...
    def get_loss(self, loss, outputs, targets, indices, num, **kwargs):
        if 'pred_hoi_logits' in outputs.keys(): # True
            loss_map = {
